...
class TestBoardInteraction(unittest.TestCase):
    ...
    def test_handle_event_mouse_button_down_on_board(self, mock_get_pos, mock_get_clicked_point):
    ...
    @patch('pygame_ui.board_interaction.BoardInteraction.get_clicked_point')
    @patch('pygame.mouse.get_pos')
    def test_handle_event_mouse_button_down_off_board(self, mock_get_pos, mock_get_clicked_point):
    ...
    @patch('pygame_ui.board_interaction.BoardInteraction.get_clicked_point')
    @patch('pygame.mouse.get_pos')
    def test_handle_event_mouse_button_down_in_button_area(self, mock_get_pos, mock_get_clicked_point):
    ...
    def setUpClass_for_clicks(cls):
        ...
        cls.inner_bottom = Config.BOARD_Y + Config.BOARD_HEIGHT - Config.BORDER_THICKNESS
        ...
        cls.left_quad_x_points = [cls.inner_left + (i * cls.point_width) + 10 for i in range(6)]
        # X-coordinates for points in right quadrant
        cls.right_quad_x_points = [cls.bar_right + (i * cls.point_width) + 10 for i in range(6)]
    ...
    def test_get_clicked_point_outside_boundaries(self):
        ...
        # Click left
        self.assertIsNone(BoardInteraction.get_clicked_point((self.inner_left - 5, self.mid_y)))
        # Click right
        self.assertIsNone(BoardInteraction.get_clicked_point((self.inner_right + 5, self.mid_y)))
        # Click top
        self.assertIsNone(BoardInteraction.get_clicked_point((Config.BAR_X + 10, self.inner_top - 5)))
        # Click bottom
        self.assertIsNone(BoardInteraction.get_clicked_point((Config.BAR_X + 10, self.inner_bottom + 5)))
    ...
    def test_get_clicked_point_top_left(self):
        ...
        for i, x in enumerate(self.left_quad_x_points):
            ...
            self.assertEqual(point, expected_points[i], f"Failed for point {expected_points[i]} at x={x}")
    ...
    def test_get_point_coords_logic(self):
        ...
        for point_num in range(24):
            with self.subTest(point=point_num):
                ...
                self.assertIsNotNone(coords, f"get_point_coords({point_num}) returned None")
