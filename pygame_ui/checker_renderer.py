...
class CheckerRenderer:
    def __init__(self) -> None:
        ...
        self._inner_bottom = Config.BOARD_Y + Config.BOARD_HEIGHT - Config.BORDER_THICKNESS
        
        # Calculate consistent point width
        left_section_width = Config.BAR_X - self._inner_left
        self._point_width = left_section_width // 6
    
    def draw(self, surface: pygame.Surface, board: Board) -> List[Tuple[int, int, int, int, str]]:
        checker_positions = []
        
        for point_index in range(24):
            checkers = board.points[point_index]
            if checkers:
                self._draw_point_checkers(surface, point_index, checkers, checker_positions)
    ...
    def _draw_point_checkers(self, surface: pygame.Surface, point_index: int, 
                             checkers: List[str], checker_positions: List[Tuple[int, int, int, int, str]]) -> None:
    ...
    def _get_point_x_center(self, point_index: int) -> float:
        ...
        if 0 <= point_index <= 5:  # Bottom right
            ...
            x = (Config.BAR_X + Config.BAR_WIDTH) + (point_in_quadrant * self._point_width)
